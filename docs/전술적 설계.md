...
사용 시기
- 여러 애그리거트를 함께 조작해야 하는 경우
- 복잡한 계산이나 알고리즘이 필요한 경우
- 외부 시스템과의 도메인 로직이 필요한 경우
...
- 예: `LoanRepository`, `MemberRepository`
#### 4.2.1 리포지토리 패턴의 목적
#### 4.2.2 애그리게이트별 리포지토리
#### 4.2.3 인터페이스와 구현 분리
#### 4.2.4 쿼리 메서드 설계
...
### 2.7 팩토리 (Factory)
- 복잡한 객체 생성 로직
- 예: `LoanFactory.create()`
#### 4.3.1 복잡한 객체 생성 문제
#### 4.3.2 팩토리 메서드 vs 팩토리 클래스
#### 4.3.3 애그리게이트 팩토리 설계

### 2.8 도메인 이벤트 (Domain Event)
- 비즈니스 사건 표현
- 예: `BookBorrowed`, `BookReturned`, `LoanOverdueDetected`
